        ...
        public static void DropDownDataValidation(String dropDownFieldName, IWebElement dropDownElement, String expectedValue)
        {
            try
            {
                ...
                if (dropDown.SelectedOption.Text.ToLower() == expectedValue.ToLower())
                {
                    ...
                    Base.test.Log(LogStatus.Fail, dropDownFieldName + " Selection unsuccessful"+ " "+ "Screenshot Image " + SaveScreenShotClass.SaveScreenshot(GlobalDefinitions.driver, "ShareSkillScreenshot"));
            }
            catch (Exception e)
            {
